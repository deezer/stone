...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dim_h, kernel_size, padding, drop_path=0.1, layer_scale_init_value=1e-1):
        super().__init__()
        self.dwconv = nn.Conv2d(
            in_channels, out_channels, kernel_size=kernel_size, padding=padding, groups=in_channels, padding_mode="replicate"
        )  # depthwise conv
        self.norm = nn.LayerNorm([dim_h, out_channels])
    ...
    def forward(self, x):
        input = x
        x = self.dwconv(x)
        x = x.permute(0, 3, 2, 1)  # (N, C, H, W) -> (N, W, H, C)
        x = self.norm(x)
        x = self.pwconv1(x)
        x = self.act(x)
        x = self.pwconv2(x)
        if self.gamma is not None:
            x = self.gamma * x
        x = x.permute(0, 3, 2, 1)  # (N, W, H, C) -> (N, C, H, W)
...
class TimeDownsamplingBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, dim_h, bias=True):
        super().__init__()
